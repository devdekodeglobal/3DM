...
export default function Preview3D({
  ...
  useEffect(() => {
    ...
    sceneRef.current.clearColor = BABYLON.Color4.FromHexString(
      colorHex.length === 7 ? colorHex + "ff" : colorHex,
    );

    // Create or update shadow catcher ground plane color
    let catcher = sceneRef.current.getMeshByName(
      "shadowCatcher",
    ) as BABYLON.Mesh;
    if (!catcher) {
      catcher = BABYLON.MeshBuilder.CreatePlane(
        "shadowCatcher",
        { size: 500 },
        sceneRef.current,
      );
      ...
      const shadowMat = new BABYLON.BackgroundMaterial(
        "shadowOnly",
        sceneRef.current,
      );
      ...
      if (
        sceneRef.current.prePassRenderer &&
        !sceneRef.current.prePassRenderer.excludedMaterials.includes(shadowMat)
      ) {
    ...
    if (
      catcher.material &&
      catcher.material instanceof BABYLON.BackgroundMaterial
    ) {
  ...
  const modelCacheRef = useRef<Map<string, Promise<BABYLON.AssetContainer>>>(
    new Map(),
  );
  const textureCacheRef = useRef<Map<string, BABYLON.Texture>>(new Map());
  const wallDecorationRegistryRef = useRef<Map<string, BABYLON.AbstractMesh[]>>(
    new Map(),
  );
  ...
  useEffect(() => {
    ...
    const flightCam = new BABYLON.UniversalCamera(
      "flightCam",
    ...
    const blueprintCam = new BABYLON.FreeCamera(
      "blueprintCam",
    ...
    (window as any).export3DModel = async (
      ...
      if (format === "obj") {
        ...
        const gltfData = await GLTF2Export.GLTFAsync(scene, "booth.gltf", {
          shouldExportNode,
        });
        return gltfData.glTFFiles["booth.gltf"];
      } else {
        const glbData = await GLTF2Export.GLBAsync(scene, "booth.glb", {
          shouldExportNode,
        });
        return glbData.glTFFiles["booth.glb"];
      }
    };
    (window as any).exportSceneToGLB = () =>
      (window as any).export3DModel("glb");
  ...
  useEffect(() => {
    ...
    if (
      lastBoothDimRef.current.w !== boothConfig.width ||
      lastBoothDimRef.current.d !== boothConfig.depth
    ) {
      ...
      if (orbitCam) {
        ...
        orbitCam.radius =
          (Math.max(boothConfig.width, boothConfig.depth) * 1.5 + 2) *
          cameraDistanceScale;
      }
    ...
    const floor = BABYLON.MeshBuilder.CreateGround(
      "floor",
      { width: boothConfig.width, height: boothConfig.depth },
      scene,
    );
    ...
    if (
      ...
      !scene.prePassRenderer.excludedMaterials.includes(gridMat)
    ) {
    ...
    if (roofConfig && roofConfig.enabled) {
      ...
      panels.forEach((panel: any, index: number) => {
        ...
        const roofPanelMesh = BABYLON.MeshBuilder.CreateBox(
          "roofPanel_" + panel.id,
          {
            width: wVal,
            height: rThickness,
      ...
      lights.forEach((light: any) => {
        ...
        const lightMat = new BABYLON.PBRMaterial(
          "ceilingLightMat_" + light.id,
          scene,
        );
        ...
        if (light.type === "circular") {
          lightMesh = BABYLON.MeshBuilder.CreateCylinder(
            "ceilingLightMesh_" + light.id,
            {
          ...
          lightMesh = BABYLON.MeshBuilder.CreateBox(
            "ceilingLightMesh_" + light.id,
            {
              width: wVal,
              height: 0.03,
  ...
  useEffect(() => {
    ...
    const scene = sceneRef.current;
    const blueprintCam = scene.getCameraByName(
      "blueprintCam",
    ...
    if (activeView === "perspective") {
      ...
      const aspect = engineRef.current
        ? engineRef.current.getAspectRatio(blueprintCam)
        : 1;
  ...
  useEffect(() => {
    ...
    scene.onPointerDown = (_evt, pickResult) => {
      ...
      if (
        pickResult.hit &&
        (pickResult.pickedMesh?.name === "floor" ||
          pickResult.pickedMesh?.name === "blueprintGrid")
      ) {
  ...
  useEffect(() => {
    ...
    const attachDragBehavior = (
      pivotMesh: BABYLON.AbstractMesh,
      elementId: string,
    ) => {
      const scene = sceneRef.current;
      if (!scene) return;
      ...
      dragBehavior.onDragEndObservable.add(() => {
        ...
        if (isRotating) {
          ...
          const pureRotY =
            pivotMesh.rotation.y - nativeOffset - facingOffsetRad;
          ...
          const snapToGrid = (val: number) =>
            Math.round(val / fineSnapSize) * fineSnapSize;
    ...
    if (modelUrl) {
      if (!scene.getMeshByName("baked_showcase_model")) {
        BABYLON.SceneLoader.ImportMeshAsync("", "", modelUrl, scene)
          ...
          .catch((err) =>
            console.error("Failed to load baked showcase model:", err),
          );
      }
      return;
    }
    ...
    for (let i = 0; i < walls.length; i++) {
      for (let j = i + 1; j < walls.length; j++) {
        ...
        let through = null,
          butt = null;
        if (isW1Horiz && !isW2Horiz) {
          through = d1;
          butt = d2;
        } else if (!isW1Horiz && isW2Horiz) {
          through = d2;
          butt = d1;
        } else {
          through = d1;
    ...
    const applyWallMaterial = (
      ...
      if (el.material === "Glass Wall") {
        ...
        if (el.material === "custom_color" || el.color) {
          ...
          if (
            !mat.albedoTexture ||
            (mat.albedoTexture as BABYLON.Texture).url !== texUrl
          ) {
    ...
    elements.forEach((el) => {
      ...
      if (registry.has(el.id)) {
        ...
        if (el.type === "wall") {
          const cutouts = (el.wallElements || []).filter((wel: any) =>
            ["door", "window"].includes(wel.type),
          );
          ...
          ["pillar", "caged-wall", "caged-panel", "panel"].includes(el.type)
        ) {
      ...
      if (registry.has(el.id) && !needsRecreate) {
        ...
        const regEntry = ASSET_REGISTRY.find(
          (a) => a.id === el.assetName,
        ) as any;
        const facingOffset =
          el.facingOffset !== undefined
            ? el.facingOffset
            : (regEntry?.facingOffset ?? 0);
        mesh.rotation.y =
          rotY +
          (mesh.metadata?.nativeOffset || 0) +
          BABYLON.Tools.ToRadians(facingOffset);
        if (el.type !== "asset") {
        ...
        if (el.type === "wall") {
          ...
          // Sync decorations (shelves, lights, banners, frames) even for existing walls
          const vScaleW = el.verticalScale || 1;
          const wallHeightW = h * vScaleW;
          const wValW = vW / PPM;
          const dValW = (el.thickness || 10) / PPM;
          const decorationsW = (el.wallElements || []).filter(
            (wel: any) => !["door", "window"].includes(wel.type),
          );
          const decorationStateW = JSON.stringify(decorationsW);

          if (
            !mesh.metadata ||
            mesh.metadata.decorationState !== decorationStateW
          ) {
            wallDecorationRegistryRef.current
              .get(el.id)
              ?.forEach((m) => m.dispose());
            ...
            decorationsW.forEach((wel: any, index: number) => {
              ...
              if (wel.type === "shelf") {
                mount = BABYLON.MeshBuilder.CreateBox(
                  "shelf_" + index,
                ...
                const lColor = BABYLON.Color3.FromHexString(
                  wel.lightColor || "#fff8e7",
                );
                const lIntensity = wel.intensity || 1.2;
                const model = wel.model || "wall_light_1";
                if (model === "wall_light_2") {
                  mount = BABYLON.MeshBuilder.CreateCylinder(
                    "light_" + index,
                    { diameter: Math.min(cutW, cutH), height: 0.05 },
                    scene,
                  );
                  ...
                  mount = BABYLON.MeshBuilder.CreateBox(
                    "light_" + index,
                    { width: cutW, height: 0.04, depth: 0.05 },
                    scene,
                  );
                } else {
                  mount = BABYLON.MeshBuilder.CreateBox(
                    "light_" + index,
                    { width: cutW, height: cutH, depth: 0.05 },
                    scene,
                  );
                }
        ...
        if (el.type === "wall") {
          ...
          const decorations = (el.wallElements || []).filter(
            (wel: any) => !["door", "window"].includes(wel.type),
          );
          ...
          if (needsRecreate || !registry.has(el.id)) {
            ...
            mesh = BABYLON.MeshBuilder.CreateBox(
              el.id,
              { width: wVal, height: wallHeight, depth: dVal },
              scene,
            );
            ...
            if (cutouts.length > 0) {
              ...
              cutouts.forEach((wel: any, index: number) => {
                ...
                const cutBox = BABYLON.MeshBuilder.CreateBox(
                  "cut",
                  { width: cutW, height: cutH, depth: cutD },
                  scene,
                );
                ...
                const cColor = wel.color
                  ? BABYLON.Color3.FromHexString(wel.color)
                  : null;
                const frameMat = new BABYLON.StandardMaterial("fmat", scene);
                frameMat.diffuseColor =
                  wel.type === "window" && cColor ? cColor : trimColor;
                frameMat.specularColor = new BABYLON.Color3(0.1, 0.1, 0.1);

                if (wel.type === "window") {
                  const fTop = BABYLON.MeshBuilder.CreateBox(
                    "ft",
                    { width: cutW, height: ft, depth: dVal + 0.02 },
                    scene,
                  );
                  fTop.position.y = cutH / 2 - ft / 2;
                  fTop.material = frameMat;
                  fTop.parent = accessoryGroup;
                  const fBot = BABYLON.MeshBuilder.CreateBox(
                    "fb",
                    { width: cutW, height: ft, depth: dVal + 0.02 },
                    scene,
                  );
                  ...
                  fBot.parent = accessoryGroup;
                  const fLeft = BABYLON.MeshBuilder.CreateBox(
                    "fl",
                    { width: ft, height: cutH - ft * 2, depth: dVal + 0.02 },
                    scene,
                  );
                  fLeft.position.x = -cutW / 2 + ft / 2;
                  fLeft.material = frameMat;
                  fLeft.parent = accessoryGroup;
                  const fRight = BABYLON.MeshBuilder.CreateBox(
                    "fr",
                    { width: ft, height: cutH - ft * 2, depth: dVal + 0.02 },
                    scene,
                  );
                  ...
                  const fLeft = BABYLON.MeshBuilder.CreateBox(
                    "dlf",
                    { width: ft, height: cutH - ft, depth: dVal + 0.02 },
                    scene,
                  );
                  fLeft.position.x = -cutW / 2 + ft / 2;
                  fLeft.position.y = -ft / 2;
                  fLeft.material = frameMat;
                  fLeft.parent = accessoryGroup;
                  const fRight = BABYLON.MeshBuilder.CreateBox(
                    "drf",
                  ...
                  fRight.position.x = cutW / 2 - ft / 2;
                  fRight.position.y = -ft / 2;
                  fRight.material = frameMat;
                  fRight.parent = accessoryGroup;

                  const leaf = BABYLON.MeshBuilder.CreateBox(
                    "dl",
                    { width: cutW - ft * 2, height: cutH - ft, depth: 0.04 },
                    scene,
                  );
                  leaf.position.y = -ft / 2;
                  const leafMat = new BABYLON.StandardMaterial(
                    "leafMat",
                    scene,
                  );
                  leafMat.diffuseColor = cColor
                    ? cColor
                    : new BABYLON.Color3(0.39, 0.26, 0.13);
          ...
          if (
            !mesh.metadata ||
            mesh.metadata.decorationState !== decorationState
          ) {
            wallDecorationRegistryRef.current
              .get(el.id)
              ?.forEach((m) => m.dispose());
          ...
          const faceMat = new BABYLON.StandardMaterial(
            el.id + "_face_mat",
            scene,
          );
          ...
          const sideMat = new BABYLON.StandardMaterial(
            el.id + "_side_mat",
            scene,
          );
          ...
          if (el.logoStyle === "chrome") {
            sideMat.specularColor = new BABYLON.Color3(1, 1, 1);
            sideMat.specularPower = 128;
          }
          if (el.logoStyle === "glowing") {
            sideMat.emissiveColor = baseColor;
          }
          ...
          if (el.svgData) {
            ...
            if (el.svgData.startsWith("data:")) {
              ...
              const blob = new Blob([el.svgData], {
            ...
            faceMat.emissiveColor =
              el.logoStyle === "glowing"
                ? new BABYLON.Color3(1, 1, 1)
          ...
          if (el.type === "pillar") {
            if (el.profile === "round") {
              mesh = BABYLON.MeshBuilder.CreateCylinder(
                el.id,
                { diameter: Math.max(w, d), height: h },
                scene,
              );
            } else {
              mesh = BABYLON.MeshBuilder.CreateBox(
            ...
            if (el.type === "caged-wall") {
              ...
              for (let i = 0; i < platesCount; i++) {
                ...
                if (orientation === "horizontal") {
                  plate = BABYLON.MeshBuilder.CreateBox(
                    plateName,
                    { width: w, height: plateThickness, depth: d },
                    scene,
                  );
                  ...
                  plate = BABYLON.MeshBuilder.CreateBox(
                    plateName,
                    { width: plateThickness, height: h, depth: d },
                    scene,
                  );
              ...
              for (let i = 0; i < platesCount; i++) {
                ...
                if (orientation === "horizontal") {
                  ...
                  plate = BABYLON.MeshBuilder.CreateBox(
                    plateName,
                    { width: w, height: h, depth: plateThickness },
                    scene,
                  );
          ...
          if (cache.has(cacheKey)) {
            cache
              .get(cacheKey)!
              .then((container) => instantiateAndSetup(container));
          } else {
            const ph = BABYLON.MeshBuilder.CreateBox(
              "ph",
              { width: el.width / PPM, height: 0.8, depth: el.height / PPM },
              scene,
            );
            ph.parent = pivot;
            ph.position.y = 0.4;
            ...
            phMat.emissiveColor = new BABYLON.Color3(0, 0.8, 1);
            phMat.alpha = 0.5;
            ph.material = phMat;

            const categoryFolder =
              el.categoryFolder || regEntry?.categoryFolder;
            const basePath = categoryFolder
              ? `/models/${categoryFolder}/`
              : "/models/";
            const ext = el.fileName?.toLowerCase().endsWith(".gltf")
              ? ".gltf"
              : ".glb";
            ...
            loadPromise
              .then((container) => instantiateAndSetup(container))
              .catch((e) => console.error("Failed to load model", cacheKey, e));
          }
        }
      }
    });
  ...
  useEffect(() => {
    ...
    const flightCam = scene.getCameraByName(
      "flightCam",
    ) as BABYLON.UniversalCamera;
    ...
    if (cameraMode === "flight") {
      ...
      orbitCam.setTarget(
        flightCam.position
          .clone()
          .add(flightCam.getForwardRay().direction.scale(5)),
      );
  ...
  const exportGLB = async () => {
    ...
    try {
      ...
      const glbData = await GLTF2Export.GLBAsync(scene, "design.glb", {
        shouldExportNode,
      });
      ...
      const blob =
        rawFile instanceof Blob
          ? rawFile
          : new Blob([rawFile], { type: "model/gltf-binary" });
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center bg-[var(--bg-base)] p-4 gap-4">
      {activeView === "perspective" && !isFullscreen && !hideControls && (
        <div className="w-full flex justify-between items-center max-w-[calc(100vh*16/9)]">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={() =>
                setCameraMode((prev) => (prev === "orbit" ? "flight" : "orbit"))
              }
              className="px-4 py-2 bg-[rgba(0,0,0,0.6)] hover:bg-[var(--sea-ink)] border border-[rgba(255,255,255,0.1)] text-xs font-bold text-white uppercase rounded-lg backdrop-blur-md transition-colors cursor-pointer"
            >
      ...
      <div
        ref={containerRef}
        className={`w-full relative overflow-hidden shadow-2xl ring-1 ring-white/10 ${
          isFullscreen
            ? "h-screen max-w-none rounded-none"
            : "max-h-full aspect-video rounded-2xl"
        }`}
      ...
      >
        ...
        {isFullscreen && activeView === "perspective" && !hideControls && (
          <>
            <div className="absolute top-4 left-4 flex gap-2">
              ...
              <button
                onClick={() =>
                  setCameraMode((prev) =>
                    prev === "orbit" ? "flight" : "orbit",
                  )
                }
                className="px-3 py-1.5 bg-[rgba(0,0,0,0.6)] hover:bg-[var(--sea-ink)] border border-[rgba(255,255,255,0.1)] text-[10px] font-bold text-white uppercase rounded-lg backdrop-blur-md transition-colors cursor-pointer"
              >
                Switch Camera
              </button>
            </div>
            ...
            <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex gap-2 bg-black/60 backdrop-blur-md p-2 rounded-2xl border border-white/10 shadow-2xl">
              <button
                onClick={() => setCameraAngle(-Math.PI / 2, 0.01)}
                className="px-3 py-1.5 hover:bg-white/10 rounded-lg text-xs font-bold text-white transition"
              >
                Top
              </button>
              <button
                onClick={() => setCameraAngle(-Math.PI / 2, Math.PI / 3)}
                className="px-3 py-1.5 hover:bg-white/10 rounded-lg text-xs font-bold text-white transition"
              >
                Front
              </button>
              ...
              <button
                onClick={() => setCameraAngle(Math.PI, Math.PI / 3)}
                className="px-3 py-1.5 hover:bg-white/10 rounded-lg text-xs font-bold text-white transition"
              >
                Left
              </button>
              <button
                onClick={() => setCameraAngle(0, Math.PI / 3)}
                className="px-3 py-1.5 hover:bg-white/10 rounded-lg text-xs font-bold text-white transition"
              >
                Right
              </button>
              ...
              <button
                onClick={() => setCameraAngle(-Math.PI / 4, Math.PI / 3)}
                className="px-3 py-1.5 hover:bg-white/10 rounded-lg text-xs font-bold text-white transition"
              >
                Iso L
              </button>
              <button
                onClick={() => setCameraAngle((-3 * Math.PI) / 4, Math.PI / 3)}
                className="px-3 py-1.5 hover:bg-white/10 rounded-lg text-xs font-bold text-white transition"
              >
                Iso R
              </button>
